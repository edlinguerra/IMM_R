---
title: 'IMM con R. Lab1: Aprendiendo a usar R con R-Studio'
...
1. Crear un usuario con contraseña en [Rstudio Cloud](https://rstudio.cloud/).
2. Luego, desde el Classroom, ir a la liga de **R Studio Cloud**.
3. Al ingresar notarán el espacio de trabajo de nombre *IMM*. Entrarán a ese espacio de trabajo y harán clic sobre la tarea asignata de nombre *Laboratorio 1*.
...
6. Acá un pequeño ejemplo, copien y peguen todo le código del recuadro de abajo en su editor de comandos. Luego, coloque el cursor al inicio de la primera línea de comando y presionen el botón **Run** en la esquina superior derecha de su achivo de códigos; alternativamente, presione **Ctrl+ENTER**. ¿Qué ocurrió? haga esto para las primeras seis líneas de comando. Cuando llegue a **p <- ggplot...** seleccione todas las líneas faltantes del código y presione Run o **Ctrl+ENTER**. ¿Qué ocurrió?
...
Llegados a este punto, vamos hacer algo de estadística descriptiva. Calculen promedio, varianza y desviación estandar a la variable largo del sepalo (**Sepal.Length**). Usen para ello los códigos suminitrados y responda las preguntas en el cuadro. 
...
4. La matriz traspuesta se calcula mediante la función `t()`. Obtén la matriz transpuesta de mat2, y explica en qué consiste trasponer una matriz.
...
10. Las siguientes instrucciones son para visualizar matrices.
    ...
    d) Finalmente llama a la libreria ggplot2 y hace el gráfico con la función 'qplot'. 
```{r, eval=FALSE}
...
Recuerde cargar las respuestas a estas preguntas en el archivo **mi_solucion_lab1.Rmd** y convertirlo con `Knit` en documento Word. Descargue el archivo y síbalo en el Google Classroom como entregable de la tarea.